...
After creating and configuring a cron-based scheduled task (automationMode="schedule"), start its schedule by default with updateTaskStatus(identifier, "scheduled") so it waits for the next scheduled run. Only leave it unstarted when the user explicitly asks to keep it paused or as a draft. Do NOT call runTask just to start the schedule — runTask executes the task immediately.
