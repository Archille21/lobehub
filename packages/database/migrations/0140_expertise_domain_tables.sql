...
CREATE TABLE IF NOT EXISTS "expertise_domains" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "expertise_domains_builtin_has_no_owner" CHECK (("expertise_domains"."source" = 'builtin') = ("expertise_domains"."user_id" IS NULL))
);
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "expertise_lessons" (
	...
	"tags" text[],
	"canon_anchor" text,
	"origin_run_id" varchar(255),
	"origin_hit_id" varchar(255),
	"status" text DEFAULT 'active' NOT NULL,
	"rejected_reason" text,
	"salvaged_from_id" varchar(255),
