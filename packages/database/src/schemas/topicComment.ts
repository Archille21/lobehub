 ...
 * Topics DO move: `AgentModel.transferAgent` and
 * `AgentGroupRepository.transferToWorkspace` rewrite `topics.workspaceId`
 * (including to NULL for personal scope); both call
 * `syncTopicCommentsOnTopicTransfer` inside the transfer transaction, which
 * moves this column (and mention rows) to the target workspace, or deletes
 * the comments when the topic leaves workspace scope entirely. Any new code
 * path that moves topics must do the same, or every workspace-filtered query
 * silently loses these rows while source-scoped reads keep leaking them.
 *
...
export const topicComments = pgTable(
  'topic_comments',
  {
     ...
     * live replies exist (content/editorData blanked, mentions dropped in the
     ...
     * reply — so `deletedAt IS NOT NULL` implies live replies exist.
     */
