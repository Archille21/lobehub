...
`--operation` and `--run` are interchangeable on `result submit`, `result list`
and `evidence list`; a round created without an operation is simply recorded as
`standalone`. **A missing operation id never means "skip the acceptance"** — it
only means you author the plan instead of discovering it.
