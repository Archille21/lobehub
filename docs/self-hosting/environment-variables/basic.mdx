...
- Type: Optional
- Description: Default hard timeout (in milliseconds) for sub-agent runs spawned by an agent (e.g. via `callSubAgent`), applied when the spawning tool call passes no explicit `timeout`. When the deadline passes, the sub-agent is interrupted and the parent resumes with a timeout result.
...
- Type: Optional
- Description: Step limit for sub-agent runs. Reaching the cap ends the run gracefully — the sub-agent is asked to summarize instead of being hard-killed. Lower it to cap execution steps on resource-constrained deployments.
- Default: `1000`
- Example: `200`
