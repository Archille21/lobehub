...
- 类型：可选
- 描述：Agent 派生的子 Agent（如通过 `callSubAgent`）的默认硬超时（毫秒），在工具调用未显式传入 `timeout` 时生效。超时后子 Agent 会被中断，父 Agent 以超时结果继续执行。
- 默认值：`7200000`（2 小时）
- 示例：`600000`
...
- 类型：可选
- 描述：子 Agent 运行的步数上限。达到上限后运行会优雅收尾 —— 子 Agent 被要求总结当前结果，而不是被强制终止。资源受限的部署可调低此值。
- 默认值：`1000`
- 示例：`200`
