...
export interface AgentSelfIterationCapabilityInput {
  ...
  /**
   * Whether the agent is the product-owned APM agent.
   */
  isLobeAiAgent: boolean;
}

/**
 * Checks whether a slug belongs to APM.
 *
 * Use when:
 * - Agent Signal must special-case the product-owned default assistant
 * - Code must avoid treating every virtual/builtin agent as APM
 *
 * Expects:
 ...
 * - APM should be managed by feature flag instead of stored chat config
 *
 * Expects:
 * - `isAgentSelfIterationFeatureEnabled` already includes the user-level feature-flag result
 *
 * Returns:
 * - `true` for APM when the feature flag is enabled
 * - `true` for non-APM only when `agentSelfIterationEnabled` is true
 */
