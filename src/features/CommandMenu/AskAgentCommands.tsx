...
const AskAgentCommands = memo(() => {
  ...
  // Check if APM matches the query
  const showLobeAI = !mentionQuery || 'APM'.includes(mentionQuery);
    ...
    <Command.Group heading={t('cmdk.mentionAgent')}>
      ...
      {showLobeAI && (
        <Command.Item
          ...
          onSelect={() => handleAgentSelect(inboxAgentId, 'APM', DEFAULT_INBOX_AVATAR)}
        >
          <Avatar emojiScaleWithBackground avatar={DEFAULT_INBOX_AVATAR} shape="square" size={18} />
          <div className={styles.itemContent}>
            <div className={styles.itemLabel}>@APM</div>
          </div>
        </Command.Item>
      )}
